...
public class Reportes {
	...
	public static String reportePoliza(HttpServletRequest request, HttpServletResponse response) 
	{
		...
		try {
			...
			String numeroPoliza = acctTrans.getString("poliza");
			String acctgTransTypeId = acctTrans.getString("acctgTransTypeId");
			Integer numeroMes = Integer.parseInt(numeroPoliza.substring(4, 6));
			String mes = Month.of(numeroMes).getDisplayName(TextStyle.FULL, new Locale("es", "ES")).toUpperCase().substring(0,3);
			String periodo = mes + "-" + numeroPoliza.substring(2, 4);

			Date fechaCont = new Date(acctTrans.getTimestamp("postedDate").getTime());
			String fechaContable = sdf.format(fechaCont).toUpperCase(Locale.ROOT);
			...
			String url = UtilValidate.isEmpty(serverRootUrl) ? "" : serverRootUrl+"/opentaps_images/opentaps_logo.png";
			
			jrParameters.put("logoUrl", url);
			jrParameters.put("TipoPoliza", tipoPoliza);
			jrParameters.put("Poliza", numeroPoliza);
			jrParameters.put("Concepto", acctTrans.getString("description"));
			jrParameters.put("periodo", periodo);
			jrParameters.put("fechaContable", fechaContable);
			jrParameters.put("tipoEvento", acctgTransTypeId);
			...
			if(Boolean.parseBoolean(agrupado)){
				...
				for(GenericValue polizas : poliza){
					...
					if (polizas.getString("clavePresupuestal") != null) {
						cp = polizas.getString("clavePresupuestal");
						cp = cp.substring(0,4) + "-" + cp.substring(4,9) + "-" + cp.substring(9,14) + "-" + cp.substring(14,18) + "-" + cp.substring(18,23)
								+ "-" + cp.substring(23,29) + "-" + cp.substring(29,30) + "-" + cp.substring(30,31) + "-" + cp.substring(31,33) + "-" + cp.substring(33);
					}

					reportLine.put("clavePresupuestal", cp);
					reportLine.put("contador", cont);
