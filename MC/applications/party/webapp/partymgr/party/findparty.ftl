...
<script>
  ...
  input_rusp.addEventListener('keyup', async function(event) {
    ...
    if (value && value.length > 0) {
      ...
      try {
        const response = await fetch(`http://30.0.0.71:3000/parties?partyId=`+value);
  ...
  const inhabilitarUsuario = async (event, estaHabilitado = true) => {
    ...
    if (partyId) {
      try {
        const response = await fetch(`http://30.0.0.71:3000/parties/` + partyId + `/users/desactivar`, {
          method: 'PUT',
          headers: {
  ...
  const habilitarUsuario = async (event) => {
    ...
    if (partyId) {
      try {
        const response = await fetch(`http://30.0.0.71:3000/parties/` + partyId + `/users/activar`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
