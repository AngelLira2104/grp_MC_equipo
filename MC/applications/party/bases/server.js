...
app.get('/historial-cambios/reciente', async (req, res) => {
  try {
    const result = await pool.query('SELECT tasa, fecha FROM historico_cambio ORDER BY fecha DESC LIMIT 1');
...
app.get("/conversiones-monedas", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        uom_id AS de_moneda,
        uom_id_to AS a_moneda,
        conversion_factor AS tasa,
        from_date AS desde,
        thru_date AS hasta
      FROM uom_conversion_dated
      ORDER BY from_date DESC
    `);
 
    if (result.rowCount === 0) {
      return res.status(404).json({ msg: "No se encontraron conversiones de moneda." });
    }
...
// Iniciar el servidor
app.listen(port, () => {
  console.log(`Servidor corriendo en http://40.0.0.71:${port}`);
});
